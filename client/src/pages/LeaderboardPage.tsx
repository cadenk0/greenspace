...
export default function LeaderboardPage() {
  ...
  const getRankIcon = (rank: number) => {
    switch (rank) {
      ...
      default: return <span className="text-sm font-bold text-muted-foreground w-6 text-center">{rank + 1}</span>;
    }
  };

  return (
    <div className="min-h-screen bg-background pb-20">
      <header className="sticky top-0 z-40 bg-background/80 backdrop-blur-md border-b border-border/50 px-6 py-4 flex items-center justify-center">
        <h1 className="text-xl font-display font-bold">Global Rankings</h1>
      </header>
        ...
        {!isLoading && sortedUsers && sortedUsers.length >= 3 && (
          <div className="flex justify-center items-end gap-4 py-8 mb-4">
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="bg-gray-300 text-white text-xs font-bold px-3 py-1 rounded-full -mt-5 relative z-10">2nd</div>
              <p className="font-bold text-sm mt-1 max-w-[80px] truncate">{sortedUsers[1].username}</p>
              <p className="text-xs font-mono font-bold text-primary">{sortedUsers[1].greenScore}</p>
            </div>
            ...
            <div className="flex flex-col items-center -mt-8">
              ...
              <div className="bg-yellow-500 text-white text-sm font-bold px-4 py-1 rounded-full -mt-6 relative z-10 shadow-md">1st</div>
              <p className="font-bold text-base mt-2">{sortedUsers[0].username}</p>
              <p className="text-sm font-mono font-bold text-primary">{sortedUsers[0].greenScore}</p>
            </div>
            ...
            <div className="flex flex-col items-center">
              ...
              <div className="bg-amber-700 text-white text-xs font-bold px-3 py-1 rounded-full -mt-5 relative z-10">3rd</div>
              <p className="font-bold text-sm mt-1 max-w-[80px] truncate">{sortedUsers[2].username}</p>
              <p className="text-xs font-mono font-bold text-primary">{sortedUsers[2].greenScore}</p>
            </div>
          </div>
        )}
              ...
              {sortedUsers?.map((user, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={cn(
                      "font-bold truncate",
                      user.id === currentUser?.id ? "text-primary" : "text-foreground"
                    )}>
                      {user.username}
